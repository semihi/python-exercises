...
import os
import numpy as np
from adaptive_detector import AdaptiveBuoyDetector

def parse_arguments():
    parser = argparse.ArgumentParser(description='Adaptively detect buoys in images with varying lighting')
...
def main():
    ...
    print(f"Processing image: {args.image} with automatic lighting detection")
    
    # Create adaptive detector and detect buoys
    adaptive_detector = AdaptiveBuoyDetector()
    (buoys, processed_images), detector, lighting_condition = adaptive_detector.detect_buoys(image)
    ...
    # Print results
    print(f"Detected {len(buoys)} buoys in {lighting_condition} lighting condition:")
    ...
    if args.show:
        ...
        binary_colored_resized = cv2.resize(binary, (new_w, new_h))
        ...
        cv2.putText(viz, f"Result ({lighting_condition})", (new_w+10, new_h+30), font, font_scale, (255, 255, 255), 2)
        ...
        cv2.namedWindow("Adaptive Buoy Detection", cv2.WINDOW_NORMAL)
        cv2.imshow("Adaptive Buoy Detection", viz)
        cv2.waitKey(0)
